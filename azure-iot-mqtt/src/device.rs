...
impl Client {
	...
	pub async fn new(
		...
		let (hub_name, device_id, _auth) = match identity {
			aziot_identity_common::Identity::Aziot(device) => (device.hub_name, device.device_id, device.auth)
		};
		...
		let key_handle = {
			key_client.load_key("device-id").await.unwrap()
		};
		...
		let c2d_prefix = format!("devices/{}/messages/devicebound/", device_id.0);
