...
pub(crate) async fn create_cert(
...

	{
		...
		for trusted_cert in trusted_certs {
			cert_store.add_cert(trusted_cert).map_err(|err| crate::Error::Internal(crate::InternalError::CreateCert(Box::new(err))))?;
		}
	}
	...
	if let Some((certs, private_key)) = client_cert {
		...
		let certs =
			openssl::x509::X509::stack_from_pem(certs)
			.map_err(|err| crate::Error::Internal(crate::InternalError::CreateCert(Box::new(err))))?;
		let mut certs = certs.into_iter();
		let client_cert = certs.next().ok_or_else(|| crate::Error::Internal(crate::InternalError::CreateCert("no client cert".into())))?;
		tls_connector.set_certificate(&client_cert).map_err(|err| crate::Error::Internal(crate::InternalError::CreateCert(Box::new(err))))?;
	...
	let (simple_enroll_request, ca_certs_request) =
		if let Some((username, password)) = basic_auth {
			...
			let simple_enroll_request = simple_enroll_request.header(hyper::header::AUTHORIZATION, &authorization_header_value);
			let ca_certs_request = ca_certs_request.header(hyper::header::AUTHORIZATION, authorization_header_value);
	...
	let (simple_enroll_response, ca_certs_response) =
		futures_util::future::join(
			get_pkcs7_response(&client, simple_enroll_request),
			get_pkcs7_response(&client, ca_certs_request),
		).await;
	let simple_enroll_response = simple_enroll_response.map_err(|err| crate::Error::Internal(crate::InternalError::CreateCert(Box::new(err))))?;
	let ca_certs_response = ca_certs_response.map_err(|err| crate::Error::Internal(crate::InternalError::CreateCert(Box::new(err))))?;
...
async fn get_pkcs7_response(
	...
	let request = request.map_err(|err| crate::Error::Internal(crate::InternalError::CreateCert(Box::new(err))))?;

	let response = client.request(request).await.map_err(|err| crate::Error::Internal(crate::InternalError::CreateCert(Box::new(err))))?;

	let (http::response::Parts { status, headers, .. }, body) = response.into_parts();
	let body = hyper::body::to_bytes(body).await.map_err(|err| crate::Error::Internal(crate::InternalError::CreateCert(Box::new(err))))?;

	if status != hyper::StatusCode::OK {
		return Err(crate::Error::Internal(crate::InternalError::CreateCert(format!(
	...
	let content_type =
		...
		.ok_or_else(|| crate::Error::Internal(crate::InternalError::CreateCert("EST response does not contain content-type header".into())))?
		.to_str()
		.map_err(|err| crate::Error::Internal(crate::InternalError::CreateCert(format!("EST response does not contain valid content-type header: {}", err).into())))?;
	if content_type != "application/pkcs7-mime" && !content_type.starts_with("application/pkcs7-mime;") {
		return Err(crate::Error::Internal(crate::InternalError::CreateCert(format!("EST response has unexpected content-type header: {}", content_type).into())));
	}
	...
	let pkcs7 = openssl::pkcs7::Pkcs7::from_pem(&pkcs7).map_err(|err| crate::Error::Internal(crate::InternalError::CreateCert(Box::new(err))))?;
	// Note: This borrows from pkcs7. Do not drop pkcs7 before this.
	let x509_stack = unsafe {
		let x509_stack = aziot_certd_pkcs7_to_x509(foreign_types_shared::ForeignType::as_ptr(&pkcs7));
		let x509_stack = x509_stack as *mut <openssl::x509::X509 as openssl::stack::Stackable>::StackType;
		let x509_stack: &openssl::stack::StackRef<openssl::x509::X509> = foreign_types_shared::ForeignTypeRef::from_ptr(x509_stack);
		x509_stack
	};

	let mut result = vec![];
	for x509 in x509_stack {
		let x509 = x509.to_pem().map_err(|err| crate::Error::Internal(crate::InternalError::CreateCert(Box::new(err))))?;
...
extern "C" {
	fn aziot_certd_pkcs7_to_x509(pkcs7: *const openssl_sys::PKCS7) -> *const openssl_sys::stack_st_X509;
}
